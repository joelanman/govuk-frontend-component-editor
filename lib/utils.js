...
// returns a usable result or undefined if there is no usable result
var sanitiseData = function (input) {
  if (input === '') {
    return sanitiseData.rejected
  }
...
sanitiseData.rejected = {ERROR: 'THIS ITEM SHOULD NEVER HAVE BEEN INCLUDED, IT SHOULD HAVE BEEN FILTERED OUT'}

var sanitiseArray = function (input) {
  const arr = input.map(sanitiseData).filter(item => item !== sanitiseData.rejected)
  arr.forEach((value, key) => {
    if (Array.isArray(value)) {
      arr[key] = sanitiseArray(value)
    } else if (typeof value === 'object') {
      arr[key] = {}
      storeData(value, arr[key])
    }
  })
  return arr
}
...
var storeData = function (inputObject, outputObject) {
  for (let key in inputObject) {
    const input = inputObject[key]
    if (Array.isArray(input)) {
      outputObject[key] = sanitiseArray(input)
    } else if (typeof input === 'object') {
      outputObject[key] = outputObject[key] || {}
      storeData(input, outputObject[key])
      if (sanitiseData(outputObject[key]) === sanitiseData.rejected) {
        delete outputObject[key]
      }
    } else {
      const output = sanitiseData(input)
      if (output !== sanitiseData.rejected) {
  ...
  //     val = val.filter((item) => item !== '_unchecked' || val !== "")
